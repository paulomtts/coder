...
class CoderConsole:
    ...
    def set_verbosity(self, level: str) -> None:
        """Set verbosity level: 'quiet', 'verbose', or 'debug'."""
    ...
    def tool_trace(
        self, tool_name: str, result_preview: str, context: str = ""
    ) -> None:
        ...
        if context:
            combined = Text.assemble(
                ...
                context,
                " -> ",
                Text(result_preview, style=self.theme.tool_result),
            )
        else:
            combined = Text.assemble(
                "[", label, "] ", Text(result_preview, style=self.theme.tool_result)
            )
        panel = Panel(
            combined,
